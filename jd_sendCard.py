...
if __name__ == '__main__':
    ...
    for idx in range(len(cks)):
        if len(all_cardtable[idx]) == 10:
            for i in range(1,9 + 1):
                try:
                    # 找没卡的
                    if len(all_cardtable[idx]) > 0:
                        if all_cardtable[idx][i] == 0:
                            printf("【{}】缺{}卡".format(str(idx), str(i)))
                            for j in range(len(cks)):
                                if len(all_cardtable[j]) > 0:
                    ...
                    printf(e)
                    # 发生异常所在的文件
                    printf(e.__traceback__.tb_frame.f_globals["__file__"])
                    # 发生异常所在的行数
                    printf(e.__traceback__.tb_lineno)
                    printf("ckidx:{} i:{} j:{}".format(str(idx), str(i), str(j)))
